...
Generic delegation authority does not identify that payload or destination, so a
Codex approval guardian can correctly fail closed even though the workflow itself
requires a reviewer.
...
for the delegated task, sent to an isolated worker on OpenAI's Codex model service.
Credentials, secrets, patient data, `.env`, and real database contents are excluded.

Automatic or nested activation does not create this consent. It may inherit consent
from a literal parent workflow only when that parent names the same payload and
destination; otherwise it asks once before the first external dispatch.
...
to be sent to the named destination. Codex-parent escalation rationale repeats the
egress contract but does not claim that assistant-authored rationale creates user
authorization.
...
* Automatic routing fails closed instead of silently borrowing that consent.
