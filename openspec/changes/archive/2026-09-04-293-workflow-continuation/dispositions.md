...
These dispositions are governed by the settled decisions in design.md; implementation does not reopen retained policy.

| Finding | Disposition | Bounded outcome |
| --- | --- | --- |
...
| 04 | Fix here | Explicit Astra executor/coordinator admission using available authoritative model metadata, independent of benchmark ladders and reviewer eligibility. Do not invent cross-family strength ordering. |
| 05 | Retain policy | Keep Full review and the validated Opus route. This Codex-parent triage uses the operator-directed documented unvalidated Terra exception; no automatic Astra reviewer promotion. |
| 06 | Fix here under the settled presentation rule | Keep the locked presentation wherever allowed; define the narrow higher-priority host-conflict path with stable question identity and required-answer waiting. No silent claim that the original layout was rendered. |
...
| 08 | Fix here | Permit a safe-start, manufactured-data behavior sweep during triage before source admission, without permitting implementation. |
...
| 18 | Fix here; external restart excluded | Single-source supported retry/fallback instructions and expose the observed CBM diagnostic through supported troubleshooting. Prefer prose guidance; the existing helper stderr correction is explicitly in scope. Do not close unrelated CBM sessions. |
