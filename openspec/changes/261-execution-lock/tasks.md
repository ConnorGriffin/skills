...
- [ ] Define the `EXECUTION LOCK v2` envelope in
...
- [ ] Extend the locate operation in
...
- [ ] Update `skills/drivers/orchestrate/SKILL.md` and
      `references/coordinator-mode.md` so `ORDER.md` and chunk prompts carry
      the lock as transport, never a second authority.
- [ ] Update `skills/drivers/epic/SKILL.md` and `docs/epic-flow.md`
      terminology for the lock protocol.
- [ ] Write the `ticket-workflow` and `planning-and-review` spec deltas in
      this change's `specs/` subtree.
- [ ] Add fail-first tests in `tests/test_ticket.py` covering one row per
...
- [ ] Run `scripts/validate.py`, the repository test command from `CLAUDE.md`,
