...
def build(out: Path):
    ...
    index=chrome("skills", "", "Overview", style=index_style) + '<p class="prose">A portable skill pack for coding agents. Twenty-eight skills that turn a vague request into tracked work, that work into a reviewed pull request, and that pull request into merged history — with the decisions written down as they are made.</p><figure class="figure"><div class="diagram-scroll">' + diagram(skills,relationships) + '</div><div class="legend"><span class="cat-workflows"><i></i>workflows</span><span class="cat-drivers"><i></i>drivers</span><span class="cat-tools"><i></i>tools</span><span>→ references</span></div><figcaption>Every skill in the pack, and every skill it names. Hover or tab to a box to isolate that skill’s references.</figcaption></figure><div class="catgrid"><div class="catcol">' + tables[0]+tables[1] + '</div><div class="catcol cat-tools">'+tables[2]+'</div></div>'+footer()
